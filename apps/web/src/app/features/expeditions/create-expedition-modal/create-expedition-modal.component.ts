...
import { addOutline, alertCircleOutline, barcodeOutline, checkmarkCircleOutline, closeOutline, downloadOutline, sendOutline, trashOutline } from 'ionicons/icons';
...
export class CreateExpeditionModalComponent implements OnInit {
  ...
  constructor() {
    addIcons({ addOutline, alertCircleOutline, barcodeOutline, checkmarkCircleOutline, closeOutline, downloadOutline, sendOutline, trashOutline });
  }
  ...
  exportCsv() {
    ...
    const location = this.locations().find((l) => l.id === this.lastSubmittedData!.fromLocationId);
    const headers = ['Producto', 'SKU', 'Barcode', 'Cantidad', 'Almacén', 'Ubicación origen', 'Notas'];

...
- Frontend tests currently cover app route smoke behavior, `roleGuard`, the shared `MovementFormModalComponent` data loading + submit validation, and `auth.service.spec.ts` (shared in-flight refresh: one request for concurrent callers, reset after settle, shared error propagation) — 10 tests across 4 spec files.

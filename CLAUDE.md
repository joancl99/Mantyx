...
- **NEXT SESSION** (refactor objectives #3–#6 are already done this session; these two optional items remain):
  1. Extract the six inline modals in `admin.component.html` (757 lines) into child components — start with a reusable `admin-catalog-modal` for the duplicated category/brand modals, then `admin-user-modal`, `admin-company-modal`, and a shared `admin-confirm-dialog` for the two toggle confirmations. No admin specs exist, so verify with `pnpm nx build web` (AOT template type-check) and ideally a manual run.
  2. Clear the ~46 `no-non-null-assertion` warnings still present in `apps/web` (same discipline as the `@CompanyId()` decorator / local-const narrowing used in the movement modal), as a separate commit.

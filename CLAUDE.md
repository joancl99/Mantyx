...
- Production Docker: `Dockerfile.api` (3-stage on Node 24 Alpine: `builder` → `prod-deps` → `runner`) + `Dockerfile.web` (Angular build → `nginx:stable-alpine-slim`). The `prod-deps` stage installs only runtime deps from the pruned `dist/apps/api/package.json` that webpack emits (`generatePackageJson: true`), using `docker/prod-deps.npmrc` (`node-linker=hoisted`) and `docker/prod-deps.pnpm-workspace.yaml` (`allowBuilds` for bcrypt/prisma); `tslib` is injected because the tsc bundle requires it but `generatePackageJson` omits it. `express`/`multer`/`tslib`/`prisma` must stay in `dependencies` or a clean build/runtime breaks. `docker-compose.prod.yml` orchestrates postgres + redis + api + web. Nginx proxies `/api`, `/uploads`, `/ws` to the API container. Entrypoint runs `prisma migrate deploy` before starting. `.dockerignore` excludes `.env*` so local secrets never enter build layers. MinIO removed — not used anywhere.

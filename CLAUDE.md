...
- **Login redesign** (this session, `apps/web/src/app/auth/auth-form.scss` + `login.component.{html,ts}`) — reworked into a dark, minimalist, mobile/PDA-first screen on the app navy (`--wh-bg`). The **logo PNG is never edited** (a recurring user rule — "un logo no se edita"); it sits untouched on a **contained rounded band** (`.brand-plate`, light `$login-band #d8e2e6`, `max-width: 320px`, centered, `border-radius: 16px`) at the top, with a visual-only `position: relative; top` nudge so the top-heavy artwork reads centered without changing the band box. Fields are **underline inputs** (no boxes, no leading icons) with uppercase amber labels and an amber focus rule; the password keeps the eye toggle. The action is a **solid amber pill** (`$login-accent`, no gradient, no shadows anywhere). The card has no visible surface — content centered, fills `100dvh` on mobile; the form is inset 18px. The accent lives in `$login-*` SCSS variables at the top of the file for one-line theming. (Iteration gotcha: padding/border on a `width:100vw` full-bleed element overflows horizontally — a contained `max-width` box avoided it; the user ultimately chose the contained rounded band over a full-bleed franja.)

- **Products audit** (this session) — visual cohesion with Inicio + permission alignment. The product list moved from separated cards to a **unified-surface list** matching the Inicio lists, via a new reusable shared modifier `.data-list--flush` (`_shared.scss`): one bordered card, rows split by hairlines, hover fills the whole row, 16px row padding. A new section header `Catálogo` + result-count pill (`.list-head`, mirroring the Inicio `.dash-section__header`/`__count`) replaced the thin `.page-context` line (title is "Catálogo" not "Productos" to avoid duplicating the toolbar title). Even spacing between top-level blocks via another reusable shared modifier `.page--stack` (flex column, single `--sp-8` gap, neutralises the stray `.filters-bar`/`.pagination` margins). Page-local in `products.component.scss`: extra top padding (`--sp-10`) between toolbar and search; `product-filters.component.scss` widened the search↔selects gap to `--sp-6`. **Permissions aligned to the backend**: `canEdit` is now `ADMIN || MANAGER` and `canDelete` is `ADMIN` (SUPERADMIN dropped — it's a platform role with `companyId = null` that never operates inside a tenant, and the API already gates `create/update` = `@Roles(ADMIN, MANAGER)`, `delete` = `@Roles(ADMIN)`); OPERATOR/VIEWER never see the create/edit/delete actions. `.data-list--flush` and `.page--stack` are opt-in shared classes ready to migrate Stock/Movements/Inventory in their own audit turns.

(Still un-started: the rest of the functional audit — Stock, Movimientos, Inventario, Recepciones, Expediciones, Almacenes, Management, Administración.)
...
- Brand assets live in `apps/web/public/brand/`: `mantyx-logo-full.png` (mantis symbol + wordmark), `mantyx-symbol.png` (mantis only), `mantyx-wordmark.png` ("Mantyx" text only). **The logo is never edited/recoloured — "un logo no se edita" is an explicit, firm user rule.** Its wordmark is dark navy (made for light backgrounds), so on the dark theme it needs a light backing rather than a CSS filter. Current usage: **login** shows the full logo, untouched, on a contained rounded light band (`.brand-plate`, see the Login redesign bullet); the **side menu** shows the wordmark on a white plate; the **favicon** (`apps/web/public/favicon.ico` + `favicon-16x16.png`/`favicon-32x32.png`/`apple-touch-icon.png`, linked in `index.html`) is generated from the green mantis symbol. Logos are plain `<img>` static assets — no ion-icon placeholder (`cube-sharp` was removed).

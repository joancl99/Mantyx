...
- Remaining cleanup opportunities (not yet done): extract the six inline modals in `admin.component.html` (757 lines) into child components — notably a reusable `admin-catalog-modal` for the duplicated category/brand modals; and clear the ~46 `no-non-null-assertion` warnings still present in `apps/web` (same `@CompanyId`-style discipline, separate commit).
